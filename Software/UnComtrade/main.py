...
class Downloader:
    def __init__(self):
        ...
        self.logging("程序开始运行!__由于UnComtrade限制,每小时每IP查询量有限:查询次数为一万次,查询数据条数为十万条,若达到限制,请一小时后重试!\n")
    ...
    def encode(self):
        try:
            ...
            if len(r_periods) != 1:
                ...
                for year in r_periods:
                    if type(year) == int:
                        url_periods.append(str(year))
                    else:
                        url = ""
                        for item in year:
                            url = url + str(item) + "%2C"
                ...
                for each_partner in url_partners:
                    for each_code in url_codes:
                        ...
                        threading.Thread(target=self.download,
                                         args=(url_type, url_freq, url_classification, each_period, each_reporter,
